...
public class PolicyAttributeTypeOfDevelopmentOfficerPage extends BasePage {
	...
	public void fillTypeOfDevelopmentOfficerAttribute(PolicyTypeOfDevOfficerEntity policyTypeOfDevOfficerEntity, CustomAssert assertReference){
	...
	private void submitTypeOfDevelopmentOfficerDetails(PolicyTypeOfDevOfficerEntity policyTypeOfDevOfficerEntity) throws InterruptedException {
		...
		if (policyTypeOfDevOfficerEntity.getBooleanValueForField("ConfigForwordButtonForCS")){		
			click(forwordButton);	
			workAroundPE(policyTypeOfDevOfficerEntity);
			if(policyTypeOfDevOfficerEntity.getStringValueForField("LOB").equalsIgnoreCase("Aviation")){
				click(forwordButton);
				Thread.sleep(3000);
				click(saveButton);			
			}
