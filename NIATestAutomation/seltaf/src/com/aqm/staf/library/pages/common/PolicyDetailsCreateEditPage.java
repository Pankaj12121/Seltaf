...
public class PolicyDetailsCreateEditPage extends BasePage {
	...
	public PolicyDetailsCreateEditPage(WebDriver driver, String pageName){
		...
		attchCoveragePremiumlink = new PageElement(By.xpath("//div//a//b[contains(text(),'Premium')]"), "member Premium link", false, WaitType.WAITFORELEMENTTOBECLICKABLE, 10);
		...
		renewedQuoteNumberField=new PageElement(By.xpath("//td//font[contains(text(),'Policy successfully renewed.Renewed Quote no:')]//following::b"), "Renewed Poilicy Number Field", false, WaitType.WAITFORELEMENTTOBEDISPLAYED, 10);
	...
	//Services  
	public void fillPolicyDetails(PolicyEntity policyEntity, CustomAssert assertReference) throws InterruptedException, ParseException{
		...
		if(policyEntity.getBooleanValueForField("ConfigPrimaryInformation")){
			if(policyEntity.getAction().equalsIgnoreCase("add") || policyEntity.getAction().equalsIgnoreCase("edit")){
				...
				if (policyEntity.getBooleanValueForField("ConfigDateOfApplication")) {
					String dateOfApplication=RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("DateOfApplication"));
				...
				if (policyEntity.getBooleanValueForField("ConfigPolicyHolderCode")) {
					clearAndSendKeys(policyHolderCodeTextField, policyEntity.getStringValueForField("PolicyHolderCode"));
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyHolderName")) {
					clearAndSendKeys(policyHolderNameTextField, policyEntity.getStringValueForField("PolicyHolderName"));
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigPolicyStartDate")) {
					policyStartDate=RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("PolicyStartDate"));
				...
				if (policyEntity.getBooleanValueForField("ConfigExpirationEndDate")) {
					...
					if(leapYear){
						if(policyEntity.getStringValueForField("PolicyDuration").equalsIgnoreCase("1")){
							...
							{
								expirationEndDate=RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("ExpirationEndDate"));
								clearAndEnterDate(expirationEndDateTextField, expirationEndDate);
							}
						}
						else{
						...
						if(policyEntity.getStringValueForField("PolicyDuration").equalsIgnoreCase("1")){
							String expirationEndDate=RandomCodeGenerator.dateGenerator("futureDate:365");
							clearAndEnterDate(expirationEndDateTextField, expirationEndDate);
						}
						else{
							String expirationEndDate=RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("ExpirationEndDate"));
				...
				if (policyEntity.getBooleanValueForField("ConfigPaymentFrequency")) {
					selectValueFromList(paymentFrequencyDropDown, policyEntity.getStringValueForField("PaymentFrequency"));
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigSumInsuredLimitsOfLiability")) {
					clearAndSendKeys(sumInsuredLimitsOfLiabilityTextField, policyEntity.getStringValueForField("SumInsuredLimitsOfLiability"));
				}
				if (policyEntity.getBooleanValueForField("ConfigInsuranceLimitsINR")) {
					clearAndSendKeys(insuranceLimitsINRTextField, policyEntity.getStringValueForField("InsuranceLimitsINR"));
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigUnadjustedDeposit")) {
					clearAndSendKeys(unadjustedDepositTextField, policyEntity.getStringValueForField("UnadjustedDeposit"));
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigPolicyEventEffectiveDate")) {
					clearAndSendKeys(eventEffectiveDateTextField, policyEntity.getStringValueForField("EventEffectiveDate"));
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigReinsuranceStatus")) {
					clearAndSendKeys(reinsuranceStatusTextField, policyEntity.getStringValueForField("ReinsuranceStatus"));
				}
				if (policyEntity.getBooleanValueForField("ConfigReinsuranceStatus")) {
					clearAndSendKeys(reinsuranceStatusTextField, policyEntity.getStringValueForField("ReinsuranceStatus"));
				...
				if (policyEntity.getBooleanValueForField("ConfigQuoteNo")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("QuoteNo"),fetchValueFromTextFields(quoteNoLabel), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyNo")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyNo"),fetchValueFromTextFields(policyNoLabel), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigAdditionalReferenceNo")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("AdditionalReferenceNo"),fetchValueFromTextFields(additionalReferenceNoLabel), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyBranch")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyBranch"),fetchValueFromTextFields(policyBranchLabel), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigDateOfApplication")) {
					assertReference.assertEquals(RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("DateOfApplication")) ,fetchValueFromTextFields(dateOfApplicationTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigProductCode")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("ProductCode"),fetchValueFromTextFields(productCodeTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigProductName")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("ProductName"),fetchValueFromTextFields(productNameTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyHolderCode")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyHolderCode"),fetchValueFromTextFields(policyHolderCodeTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyHolderName")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyHolderName"),fetchValueFromTextFields(policyHolderNameTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyDuration")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyDuration"),fetchValueFromTextFields(policyDurationTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyDurationUnit")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyDurationUnit"),fetchValueFromList(policyDurationUnitDropDown), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPolicyStartDate")) {
					assertReference.assertEquals(RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("PolicyStartDate")) ,fetchValueFromTextFields(policyStartDateTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigExpirationEndDate")) {
					int year = Calendar.getInstance().get(Calendar.YEAR);
					Boolean  leapYear=RandomCodeGenerator.determineLeapYear(year);
					if(leapYear){
						if(policyEntity.getStringValueForField("PolicyDuration").equalsIgnoreCase("1")){
							if(policyEntity.getStringValueForField("ExpirationEndDate").contains("futureDate:364")||policyEntity.getStringValueForField("ExpirationEndDate").contains("futureDate:365")){
								assertReference.assertEquals(RandomCodeGenerator.dateGenerator("futureDate:364") ,fetchValueFromTextFields(expirationEndDateTextField), AssertionType.WARNING);
							}
							else{
								assertReference.assertEquals(RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("ExpirationEndDate")) ,fetchValueFromTextFields(expirationEndDateTextField), AssertionType.WARNING);
							}
						}
						else{
							assertReference.assertEquals(RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("ExpirationEndDate")) ,fetchValueFromTextFields(expirationEndDateTextField), AssertionType.WARNING);
						...
						if(policyEntity.getStringValueForField("PolicyDuration").equalsIgnoreCase("1")){
							assertReference.assertEquals(RandomCodeGenerator.dateGenerator("futureDate:365") ,fetchValueFromTextFields(expirationEndDateTextField), AssertionType.WARNING);
						}
						else{
							assertReference.assertEquals(RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("ExpirationEndDate")) ,fetchValueFromTextFields(expirationEndDateTextField), AssertionType.WARNING);
						}
				...
				if (policyEntity.getBooleanValueForField("ConfigPolicyStartTime")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PolicyStartTime"),fetchValueFromTextFields(policyStartTimeTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigPaymentFrequency")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PaymentFrequency"),fetchValueFromList(paymentFrequencyDropDown), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigBusinessType")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("BusinessType"),fetchValueFromList(businessTypeDropDown), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigCurrency")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("Currency"),fetchValueFromTextFields(currencyTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigCurrencyRate")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("CurrencyRate"),fetchValueFromTextFields(currencyRateTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigSumInsuredLimitsOfLiability")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("SumInsuredLimitsOfLiability"),fetchValueFromTextFields(sumInsuredLimitsOfLiabilityTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigInsuranceLimitsINR")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("InsuranceLimitsINR"),fetchValueFromTextFields(insuranceLimitsINRTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigGrossPremium")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("GrossPremium"),fetchValueFromTextFields(grossPremiumTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigAdjustments")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("Adjustments"),fetchValueFromTextFields(adjustmentsTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigDeduction")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("Deduction"),fetchValueFromTextFields(deductionTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigServiceTax")) {

					if(policyEntity.getStringValueForField("ServiceTax").equalsIgnoreCase("0.00")){
						assertReference.assertEquals(policyEntity.getStringValueForField("ServiceTax"),fetchValueFromTextFields(serviceTaxTextField), AssertionType.WARNING);
					}
					else{
						...
						serviceTax=RandomCodeGenerator.serviceTaxCalculation(grossPremium);

						assertReference.assertEquals(serviceTax,fetchValueFromTextFields(serviceTaxTextField), AssertionType.WARNING);
					}

				}

				if (policyEntity.getBooleanValueForField("ConfigUnadjustedDeposit")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("UnadjustedDeposit"),fetchValueFromTextFields(unadjustedDepositTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigStampDuty")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("StampDuty"),fetchValueFromTextFields(stampDutyTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigAdminFees")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("AdminFees"),fetchValueFromTextFields(adminFeesTextField), AssertionType.WARNING);
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigNetPremiumAmt")) {
					...
					if(!(policyEntity.getStringValueForField("ServiceTax").isEmpty())){
						if(policyEntity.getStringValueForField("ServiceTax").equalsIgnoreCase("0.00")){
							assertReference.assertEquals(policyEntity.getStringValueForField("NetPremiumAmt"),fetchValueFromTextFields(netPremiumAmtTextField), AssertionType.WARNING);
						}
						else{
							String netPremiumAmount=RandomCodeGenerator.formatNumber(2, (Double.parseDouble(grossPremium)+Double.parseDouble(serviceTax)));
							assertReference.assertEquals(netPremiumAmount,fetchValueFromTextFields(netPremiumAmtTextField), AssertionType.WARNING);
						}
					}
				}

				if (policyEntity.getBooleanValueForField("ConfigPremiumCurrency")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PremiumCurrency"),fetchValueFromTextFields(premiumCurrencyTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigPremiumCurrencyRate")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("PremiumCurrencyRate"),fetchValueFromTextFields(premiumCurrencyRateTextField), AssertionType.WARNING);
				}
				...
				if (policyEntity.getBooleanValueForField("ConfigNetPremiumINR")) {
					...
					if(!(policyEntity.getStringValueForField("ServiceTax").isEmpty())){
						if(policyEntity.getStringValueForField("ServiceTax").equalsIgnoreCase("0.00")){
							assertReference.assertEquals(policyEntity.getStringValueForField("NetPremiumINR"),fetchValueFromTextFields(netPremiumINRTextField), AssertionType.WARNING);
						}
						else{
							String netPremiumAmountINR=RandomCodeGenerator.formatNumber(2, (Double.parseDouble(grossPremium)+Double.parseDouble(serviceTax)));
							assertReference.assertEquals(netPremiumAmountINR,fetchValueFromTextFields(netPremiumAmtTextField), AssertionType.WARNING);
						}
					}
				}

				if (policyEntity.getBooleanValueForField("ConfigEventEffectiveDate")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("EventEffectiveDate"),fetchValueFromTextFields(eventEffectiveDateTextField),AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigLatestEventName")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("LatestEventName"),fetchValueFromTextFields(latestEventNameTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigCurrentStatus")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("CurrentStatus"),fetchValueFromTextFields(currentStatusTextField), AssertionType.WARNING);
				}
				if (policyEntity.getBooleanValueForField("ConfigReinsuranceStatus")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("ReinsuranceStatus"),fetchValueFromTextFields(reinsuranceStatusTextField), AssertionType.WARNING);
				}
				/*if (policyEntity.getBooleanValueForField("ConfigProposalPolicyNo")) {
					actualPolicyEntity.setStringValueForField("ProposalPolicyNo", fetchValueFromTextFields(proposalPolicyNoTextField));
					assertReference.assertEquals(policyEntity.getStringValueForField("ProposalPolicyNo"),actualPolicyEntity.getStringValueForField("ProposalPolicyNo"), AssertionType.WARNING);
				}*/
				if (policyEntity.getBooleanValueForField("ConfigMaximumBenefitAmount")) {
					assertReference.assertEquals(policyEntity.getStringValueForField("MaximumBenefitAmount"),fetchValueFromTextFields(maximumBenefitAmountINRTextField), AssertionType.WARNING);
				}

				if (policyEntity.getBooleanValueForField("ConfigUserId")) {
					switchToFrame("head");
					assertReference.assertEquals(policyEntity.getStringValueForField("UserId"),fetchValueFromFields(userIdLabel), AssertionType.WARNING);
					switchToFrame("display");
				}
				if (policyEntity.getBooleanValueForField("ConfigRole")) {
					switchToFrame("head");
					assertReference.assertEquals(policyEntity.getStringValueForField("Role"),fetchValueFromFields(roleLabel), AssertionType.WARNING);
					switchToFrame("display");
				}
	...
	public void fillReason(PolicyEntity policyEntity) throws InterruptedException{
		...
		if(policyEntity.getBooleanValueForField("ConfigManualRefundPremium")){
			...
			else if((policyEntity.getStringValueForField("LOB").equalsIgnoreCase("MiscNonLiability")) && ((policyEntity.getStringValueForField("Product").equalsIgnoreCase("EV")) || (policyEntity.getStringValueForField("Product").equalsIgnoreCase("EX"))||  (policyEntity.getStringValueForField("Product").equalsIgnoreCase("HH"))||(policyEntity.getStringValueForField("Product").equalsIgnoreCase("SH"))||(policyEntity.getStringValueForField("Product").equalsIgnoreCase("JI")))){
				double manualRefundPremium;
				manualRefundPremium = Double.parseDouble(fetchValueFromTextFields(manualRefundPremiumTextField));
		...
		/*if (policyEntity.getBooleanValueForField("ConfigEventEffectiveDate")) {
			click(eventEffectiveDateFindButton);
			Thread.sleep(1000);
			selectDateFromCalender(eventEffectiveDateCancelTextField, policyEntity.getStringValueForField("EventEffectiveDate"));
			switchToFrame("display");
		}*/

		if (policyEntity.getBooleanValueForField("ConfigEndorseEffectiveDate")){
			String endorseeffectiveDate=RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("EndorseEffectiveDate"));
		...
		if(policyEntity.getBooleanValueForField("ConfigPassportDetailsVerified")){
			selectValueFromList(passportDetailsVerifiedDropdown, policyEntity.getStringValueForField("PassportDetailsVerified"));
		}
		if(policyEntity.getBooleanValueForField("ConfigCalculatedRefundPremium")){
			clearAndSendKeys(calculatedRefundPremiumTextField, policyEntity.getStringValueForField("CalculatedRefundPremium"));
		}
		if(policyEntity.getBooleanValueForField("ConfigPremiumCalculationBasis")){
			selectValueFromList(premiumCalculationBasisDropdown, policyEntity.getStringValueForField("PremiumCalculationBasis"));
		}
	...
	public void closeRenewWindow(PolicyEntity policyEntity) throws InterruptedException{
		if(policyEntity.getBooleanValueForField("ConfigClose")){
			...
			if(!(policyEntity.getStringValueForField("LOB").equalsIgnoreCase("Engineering")) && (policyEntity.getStringValueForField("Product").equalsIgnoreCase("ME")))
			{	
	...
	public void fillReasonPageOnCollectionPage(PolicyEntity policyEntity) throws InterruptedException{
		...
		if(policyEntity.getBooleanValueForField("ConfigLastPaymentDueDate")){
			String lastPaymentDueDate=RandomCodeGenerator.dateGenerator(policyEntity.getStringValueForField("LastPaymentDueDate"));
		...
		if(policyEntity.getBooleanValueForField("ConfigReasonForInitiate")){
			clearAndSendKeys(reasonForInitiatingThisActionTextField,policyEntity.getStringValueForField("ReasonForInitiate") );
		}
	...
	public void selectPremiumCurrency(PolicyEntity policyEntity) throws InterruptedException{
		...
		premiumCurrencyToBeSelectedLink=new PageElement(By.xpath("//a[contains(text(),'"+policyEntity.getStringValueForField("PremiumCurrency")+"')]"),"Premium Currency to be Selected", false, WaitType.WAITFORELEMENTTOBECLICKABLE, 10);
		click(premiumCurrencyToBeSelectedLink);
		switchToWindow();
		switchToFrame("display");
	}

	public void clickPolicyCollectionDetails(PolicyEntity policyEntity,CustomAssert assertReference) throws InterruptedException {
	...
	public void clickTagNoChangeButton(PolicyEntity policyEntity,CustomAssert assertReference) throws InterruptedException {
		if(policyEntity.getAction().equalsIgnoreCase("add")||policyEntity.getAction().equalsIgnoreCase("edit")){
