...
public class ClaimAttributesProceedCourtMasterForAllProductPage extends BasePage{
	...
	public ClaimAttributesProceedCourtMasterForAllProductPage(WebDriver driver,String pageName) {
		...
		courtCodeTextField = new PageElement(By.xpath("//td//div[contains(text(),'Court Code')]/following::input[1]"), "Court Code TextField", false, WaitType.WAITFORELEMENTTOBEDISPLAYED, 10);
		caseNumberTextField = new PageElement(By.xpath("//td//div[contains(text(),'Case Number')]/following::input[1]"), "Case Number TextField", false, WaitType.WAITFORELEMENTTOBEDISPLAYED, 10);
	...
	public void fillCourtMasterForAllProductDetails(ClaimAttriCourtMasterEntity claimAttriCourtMasterEntity, CustomAssert assertReference){
		if(claimAttriCourtMasterEntity.getAction().equalsIgnoreCase("verify")){
			...
			if (claimAttriCourtMasterEntity.getBooleanValueForField("ConfigCourtCode")) {
				assertReference.assertEquals(claimAttriCourtMasterEntity.getStringValueForField("CourtCode"),fetchValueFromFields(courtCodeTextField), AssertionType.WARNING);
			}
			if (claimAttriCourtMasterEntity.getBooleanValueForField("ConfigCaseNo")) {
				assertReference.assertEquals(claimAttriCourtMasterEntity.getStringValueForField("CaseNo"),fetchValueFromFields(caseNumberTextField), AssertionType.WARNING);
			}
