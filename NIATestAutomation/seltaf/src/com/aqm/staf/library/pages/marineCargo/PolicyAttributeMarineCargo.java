...
public class PolicyAttributeMarineCargo extends BasePage{
	...
	public PolicyAttributeMarineCargo(WebDriver driver, String pageName) {
		...
		reasonForServiceTaxExemptionDropDown=new PageElement(By.xpath("//td//div[contains(text(),'Reason for Service Tax Exemption')]/following::select"), "Reason for Service Tax Exemption", false, WaitType.WAITFORELEMENTTOBEEENABLED, 10);
	...
	public void fillPolicyAttributeMarineCargo(PolAttrMarinCargoEntity polAttrMarinCargoEntity, CustomAssert assertReference) throws InterruptedException{
		if(polAttrMarinCargoEntity.getAction().equalsIgnoreCase("add") || polAttrMarinCargoEntity.getAction().equalsIgnoreCase("edit")){
			//DIGVIJAY PROD AT
			if (polAttrMarinCargoEntity.getBooleanValueForField("ConfigIsServiceTaxExempted")) {
				selectValueFromList(isServiceTaxExemptedDropDown, polAttrMarinCargoEntity.getStringValueForField("IsServiceTaxExempted"));
			}
			if (polAttrMarinCargoEntity.getBooleanValueForField("ConfigReasonForServiceTaxExemption")) {
				selectValueFromList(reasonForServiceTaxExemptionDropDown, polAttrMarinCargoEntity.getStringValueForField("ReasonForServiceTaxExemption"));
			}
			...
			//DIGVIJAY PROD AT
			if (polAttrMarinCargoEntity.getBooleanValueForField("ConfigIsServiceTaxExempted")) {
				assertReference.assertEquals(polAttrMarinCargoEntity.getStringValueForField("IsServiceTaxExempted"), fetchValueFromList(isServiceTaxExemptedDropDown), AssertionType.WARNING);
			}
			if (polAttrMarinCargoEntity.getBooleanValueForField("ConfigReasonForServiceTaxExemption")) {
				assertReference.assertEquals(polAttrMarinCargoEntity.getStringValueForField("ReasonForServiceTaxExemption"), fetchValueFromList(reasonForServiceTaxExemptionDropDown), AssertionType.WARNING);
			}
			if (polAttrMarinCargoEntity.getBooleanValueForField("ConfigNameOfSEZLocation")) {
	...
	public void clickNextButtonPolicyAttributeCDPolicyDetails(PolAttrMarinCargoEntity polAttrMarinCargoEntity){
		if (polAttrMarinCargoEntity.getBooleanValueForField("ConfigForwordButton")){			
			click(forwardButton);
			switchToFrame("display");
	...
	public void fillandSubmitPolicyInsIntAviation(PolAttrMarinCargoEntity polAttrMarinCargoEntity, CustomAssert assertReference) throws InterruptedException {
		if(isConfigTrue(polAttrMarinCargoEntity.getConfigExecute())){
			...
			clickNextButtonPolicyAttributeCDPolicyDetails(polAttrMarinCargoEntity);
