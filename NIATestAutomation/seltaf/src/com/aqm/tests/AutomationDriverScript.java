...
public class AutomationDriverScript extends FunctionalKeywords {
	...
	public void scriptExecutioner(String platform, String browserName, String browserVersion, String testPlan_Reference, String executionMasterTestSuite_Reference, 
		...
		try {
			...
			for (ExecutionTestSteps executionTestStep : executionTestSteps) {
				...
				try {
					...
					/*if (assertReference.allStepsSkip()) {
						...
						new ScriptExecutionException(" Step Skipped By the Scenario...Kindly Check ");
					}*/
				...
				}else if (testStep_ExecutionStatus == 4) {
					if (!ExecutionError){
			...
			if(executionTestScenario_ExecutionStatus == 4){
				
				if (ExecutionError){
					sro.setTypeOfFailure("Failed");
				}
			...
			Long exeTime= ScenarioWatch.getTime();
			...
			if (!(driver == null)) {
				...
				try{
					if (assertReference.allStepsSkip()){
						...
						FrameworkServices.logMessage("All Steps Been Skipped for this scenario");
