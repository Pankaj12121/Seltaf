...
public class PartyFollowupDetails extends BasePage {
	...
	public PartyFollowupDetails(WebDriver driver, String pageName) {
		...
		eventCodeTextField = new PageElement(By.name("pEventCode"),"EventCode Text Field", false,WaitType.WAITFORELEMENTTOBEDISPLAYED,10);
		jobStatusLabel = new PageElement(By.xpath(genericLocatorforLabel("FollowupDetail", "Status")),"Job Status Label", false, WaitType.WAITFORELEMENTTOBEDISPLAYED,10);
	...
	public void fillPartyFollowupDetails(PartyFollowupDetailsEntity partyFollowupDetailsEntity, CustomAssert assertReference){
		if(partyFollowupDetailsEntity.getAction().equalsIgnoreCase("add") || partyFollowupDetailsEntity.getAction().equalsIgnoreCase("edit")){
			...
			if (partyFollowupDetailsEntity.getBooleanValueForField("ConfigFollowupDate")) {
				assertReference.assertEquals(partyFollowupDetailsEntity.getStringValueForField("FollowupDate"), fetchValueFromTextFields(followupDateTextField), AssertionType.WARNING);
			}
			...
			if (partyFollowupDetailsEntity.getBooleanValueForField("ConfigJobStatus")) {
				assertReference.assertEquals(partyFollowupDetailsEntity.getStringValueForField("JobStatus"), fetchValueFromTextFields(jobStatusLabel), AssertionType.WARNING);
			}
			if (partyFollowupDetailsEntity.getBooleanValueForField("ConfigKeyword")) {
