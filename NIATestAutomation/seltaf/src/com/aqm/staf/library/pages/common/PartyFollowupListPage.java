...
public class PartyFollowupListPage extends BasePage{
	...
	public void selectFollowUpFromSearchPage(PartyFollowupDetailsEntity partyFollowupDetailsEntityData){
		if (partyFollowupDetailsEntityData.getBooleanValueForField("ConfigSearchDate")||! partyFollowupDetailsEntityData.getAction().equalsIgnoreCase("add")){
			String followupDate=partyFollowupDetailsEntityData.getStringValueForField("Date");
			followupOnSearchPage = new PageElement(By.linkText(followupDate), "Date in Search Panel", false, WaitType.WAITFORELEMENTTOBEEENABLED, 10);
			click(followupOnSearchPage);
			switchToFrame("display");
